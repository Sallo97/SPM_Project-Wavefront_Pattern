...
struct VecMatrix {
     ...
     */
    explicit VecMatrix(const int vec_length)
        :rows_vector(vec_length, std::vector<double>(1, 0)),
         cols_vector(vec_length, std::vector<double>(1, 0)){
        // Each sub-vector starts with length 1 and value (i+1)/n
    ...
    */
    void PutElement(ElemInfo x, double val) {
        // Resize the vector
        rows_vector.resize(rows_vector.size() + 1);
        cols_vector.resize(cols_vector.size() + 1);
    ...
    // Parameters
    double_vec rows_vector;
    double_vec cols_vector;
};

/**
 * @brief Execute a DotProduct computation over two input vectors.
 * @param[in] fst_vec = reference to the first vector
 * @param[in] snd_vec = reference to the second vector
 * @param[out] result = reference to the result
 */
double DotProductComputation(const std::vector<double>& fst_vec,
                             const std::vector<double>& snd_vec,
                             double& result) {
    for(int i = 0; i < fst_vec.size(); ++i)
        result += fst_vec[i] * snd_vec[i];
    return result;
}
...
void MasterJob(int& my_rank, int& num_processes, int& row_length,
    ...
    // Initializing Matrix and setting params
    VecMatrix mtx(row_length);
    int diag_length{0};
    int limit{3}; // TODO GIVE IT AS A CMD ARGUMENT
    double val{1}; // TODO METTILO A 0


    // Starting Wavefront Computation
    for (int current_diag = 1; current_diag < row_length; ++current_diag) {

        diag_length = row_length - current_diag;
        // Computing each element of the diagonal
        for(int k = 0; k < diag_length; ++k) {
            ElemInfo x{k, current_diag + k};

            // diag_length determines the number of elements of
            // the row vector and col vector for that element
            if(diag_length <= limit) { // Sequential DotProduct
                std::vector<double>& row = mtx.GetRow(x.idx_row);
                std::vector<double>& col = mtx.GetCol(x.idx_col);
                DotProductComputation(row, col, val);
            }
            else {                    // Parallel DotProduct
                ParallelDotProduct(diag_length, num_processes, mtx,
                                   x, local_row, local_col, val);
            }

            // Storing the element in the matrix
            mtx.PutElement(x, std::cbrt(val));
            val = 1; //TODO METTILO A 0
        }
    }

    // Signaling Workers that they have to end
    int end_msg = EOS_MESSAGE;
    MPI_Bcast(&end_msg, 1, MPI_INT, MASTER_RANK, MPI_COMM_WORLD);
}
...
void WorkerJob(int& my_rank, int& row_length, double& local_sum,
    ...
    while(!end_worker) {
        // Receiving length/EOS
        MPI_Bcast(&length, 1, MPI_INT, 0, MPI_COMM_WORLD);
        if (length == EOS_MESSAGE)
            end_worker = true;
